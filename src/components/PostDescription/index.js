...
import { EditInput, EditForm, Description } from "./style";
import { api } from "../../services";
import { useAuth, useTrending } from "../../providers";

async function submit(e, requestData, updateTrending) {
  e.preventDefault();
  const {
    isLoading,
    setLoading,
  ...
  const currentHashtags = newDescription
    .split(" ")
  ...
  currentHashtags.forEach((hashtag) => {
    if (!originalHashtags.find((h) => h === hashtag)) newHashtags.push(hashtag);
  });
...
export default function PostDescription({ data, editState }) {
  ...
  const { updateTrending } = useTrending();
  ...
  const handleKeyUp = (e) => {
    if (e.key === "Escape") editState.setEditing(!editState.isEditing);
  };
  ...
  return editState.isEditing ? (
    <EditForm onSubmit={(e) => submit(e, requestData, updateTrending)}>
      <EditInput
        ref={inputRef}
        type="text"
        value={value}
